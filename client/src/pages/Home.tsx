...
async function copyProductInfo(product: Product) {
  ...
  const text = `👟 Modelo: ${product.title} | Marca: ${product.vendor || "JAMESSPORT"} | Precio: ${formatMoney(product.priceRange.min.amount, product.priceRange.min.currencyCode)} | Tallas: ${sizes} | 🖼️ Foto: ${product.images[0]?.url || ""}`;
  ...
  return (
    <>
      <div className="announcement-bar">
        ...
        <span>SNEAKERS URBANAS. ESTILO QUE COMPITE.</span>
      </div>
      <header className="site-header">
        <div className="header-inner">
          ...
          <Link href="/" className="wordmark" aria-label="JAMESSPORT, inicio">
            JAMESSPORT<span className="wordmark-dot">.</span>
          </Link>
    ...
    <section className="hero-section">
      ...
      <div className="hero-visual">
        ...
        <div className="hero-image-wrap">
          <ImageFrame src={image} alt={featuredProduct?.title || "Sneaker JAMESSPORT"} priority />
  ...
  return (
    <article className="product-card">
      ...
      <div className="product-meta">
        <div>
          <p className="product-category">{product.vendor || "JAMESSPORT"} / {product.productType || "Sneakers"}</p>
...
function Footer() {
  return <footer className="site-footer"><div className="footer-top"><div><Link href="/" className="wordmark footer-wordmark">JAMESSPORT<span className="wordmark-dot">.</span></Link><p className="footer-tagline">Tu ritmo.<br />Tu par.</p></div><div className="footer-column"><span className="footer-label">Explore</span><a href="#shop">Shop all</a><a href="#story">Our story</a><a href="#journal">Journal</a></div><div className="footer-column"><span className="footer-label">Care</span><a href="#shipping">Shipping & returns</a><a href="#size">Size guide</a><a href="#care">Product care</a></div><div className="footer-newsletter"><span className="footer-label">Stay sharp</span><p>Notas de la colección, directo a tu inbox.</p><form onSubmit={event => { event.preventDefault(); toast.success("You're on the list", { description: "Mira tu inbox para la bienvenida." }); }}><input type="email" placeholder="Tu email" required aria-label="Correo electrónico" /><button type="submit" aria-label="Suscribirse"><ArrowUpRight size={17} /></button></form><div className="footer-social"><a href="#instagram" aria-label="Instagram"><Instagram size={17} /></a><a href="#mail" aria-label="Email"><Mail size={17} /></a></div></div></div><div className="footer-bottom"><span>© 2026 JAMESSPORT</span><span>Diseñado por JAMESSPORT</span><span>Privacy · Terms · Cookies</span></div></footer>;
}
